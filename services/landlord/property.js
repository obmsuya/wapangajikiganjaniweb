    ...
    try {
      ...
      if (response.property_floor && Array.isArray(response.property_floor)) {
        ...
        response.property_floor = response.property_floor.map(floor => {
          // Normalize floor_no: mobile sends 1-indexed, web sends 0-indexed
          // Rule: if floor_no >= totalFloors, it is 1-indexed → convert to 0-indexed
          const normalizedFloorNo = floor.floor_no >= totalFloors
            ? floor.floor_no - 1
            : floor.floor_no;

          return {
            ...floor,
            floor_no: normalizedFloorNo,   // overwrite with normalized value
