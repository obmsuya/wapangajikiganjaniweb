...
export default function LandlordSubscriptionsList() {
  ...
  const summary = useMemo(() => {
    ...
    const expiring = tableData.filter((r) => {
      const d = daysUntil(r.subscription?.end_date);
      return (
        r.subscription?.status === "active" && d !== null && d > 0 && d <= 7
      );
    }).length;
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <SortButton column={column} label="Properties" />
        ),
        cell: ({ row }) => {
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setLocalFilters((p) => ({ ...p, plan_type: v }))
                    }
                  >
                    <SelectTrigger className="h-8 text-sm">
