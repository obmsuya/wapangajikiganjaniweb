        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-lg border bg-card p-4 space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span>
                {fmtDate(schedule.period_start)} –{" "}
                {fmtDate(schedule.period_end)}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span
                className={
                  isOverdue && !isPaid ? "text-destructive font-medium" : ""
                }
              >
...
export default function TenantRentSchedule({ onPayNow }) {
  ...
  const SortIcon = ({ field }) => {
    if (sortField !== field)
      return (
        <ChevronsUpDown className="ml-1 h-3.5 w-3.5 text-muted-foreground inline" />
      );
    return sortDir === "asc" ? (
    ...
    if (schedule.days_overdue > 0)
      return (
        <Badge variant="destructive" className="gap-1">
          ...
          Overdue {schedule.days_overdue}d
        </Badge>
      );
    return (
      <Badge variant="secondary" className="gap-1">
            ...
            {[
              {
                label: "Paid",
                count: paid.length,
                total: paid.reduce((s, r) => s + r.rent_amount, 0),
                variant: "outline",
              },
              {
                label: "Due",
                count: due.length,
                total: due.reduce((s, r) => s + r.rent_amount, 0),
                variant: "secondary",
              },
              {
                label: "Overdue",
                count: overdue.length,
                total: overdue.reduce((s, r) => s + r.rent_amount, 0),
          ...
          <CardContent className="p-0">
            {rentSchedules.length === 0 ? (
              <div className="text-center py-16 text-muted-foreground px-6">
                ...
                <p className="text-sm">
                  Your rent schedule will appear here once set up by your
                  landlord.
                </p>
              </div>
            ) : (
                  ...
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>
                        <button
                          className="flex items-center font-medium"
                          onClick={() => toggleSort("rent_amount")}
                        >
                            ...
                            <p className="text-xs text-muted-foreground font-normal">
                              {formatCurrency(
                                schedule.amount_remaining ??
                                  schedule.rent_amount - schedule.amount_paid,
                              )}{" "}
                              left
                            </p>
                          )}
