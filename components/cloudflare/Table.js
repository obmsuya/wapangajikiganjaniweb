  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="flex flex-col justify-between items-start my-8 gap-4 w-full">
                    ...
                    <div key={index} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleColumnVisibility(index, checked)
                        }
                      />
                      <label
                        htmlFor={`col-${index}`}
        ...
        {/* Pagination controls */}
        {pagination && (
          <div className="flex items-center space-x-2 w-full justify-between max-md:flex-col max-md:gap-6">
            <div className="space-x-2 max-md:flex max-md:justify-between max-md:items-center max-md:w-full">
              <span className="text-sm text-gray-500">
                Showing{" "}
                {sortedData.length ? (currentPage - 1) * rowsPerPage + 1 : 0}-
                {Math.min(currentPage * rowsPerPage, sortedData.length)} of{" "}
                {sortedData.length}
              </span>

              <select
                className="border border-gray-300 rounded-full text-sm px-2 py-2"
                value={rowsPerPage}
                onChange={(e) => {
                  ...
                  .map((column, index) => (
                    ...
                    >
                      <div className="flex items-center space-x-1">
                        <span className="max-md:text-sm">{column.header}</span>
                        {column.sortable &&
