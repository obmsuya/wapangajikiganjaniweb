...
export const usePartnerStore = create((set, get) => ({
      ...
      if (response && !response.error) {
        const formattedData = {
          ...
          walletSummary: {
            ...
            minimumPayout: parseFloat(response.wallet_summary?.minimum_payout || 5000)
          },
          referralStats: {
            ...
            thisMonthReferrals: parseInt(response.referral_stats?.this_month_referrals || 0)
          },
    ...
    try {
      ...
      if (response && !response.error) {
        const formattedStats = {
          ...
          recentReferrals: (response.recent_referrals || []).map(referral => ({
    ...
    try {
      ...
      if (response && !response.error) {
        const formattedData = {
          ...
          recentTransactions: (response.recent_transactions || []).map(transaction => ({
            id: transaction.id,
  ...
  fetchPayoutHistory: async (limit = 20) => {
    try {
      ...
      if (response && !response.error) {
        const formattedHistory = (response.disbursements || []).map(payout => ({
          id: payout.id,
  ...
  checkPayoutEligibility: async () => {
    try {
      ...
      const errorMessage = error.message || 'Failed to check payout eligibility';
